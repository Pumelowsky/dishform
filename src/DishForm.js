...
const DishForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!validateForm()) {
      return; // If form is incorrect it break function
    }
    try {
      const formData = {
        name,
        ...
        type: dishType,
        no_of_slices: 
          dishType === "pizza" ? parseInt(noOfSlices) : null,
        diameter:
          dishType === "pizza" ? parseFloat(diameter) : null,
        spiciness_scale: 
          dishType === "soup" ? parseInt(spicinessScale) : null,
        slices_of_bread:
          dishType === "sandwich" ? parseInt(slicesOfBread) : null,
      };

      // Send data to API via POST (axios)
  ...
  return (
    <Form onSubmit={handleSubmit}>
      <h1 className="mb-3">Dish Form</h1>
      <h6 className="mb-3 text-muted">
        Form will send data to API and results will be printed
      </h6>
      <Row className="mb-3">
